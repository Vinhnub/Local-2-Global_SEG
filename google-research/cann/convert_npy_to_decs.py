...
def convert_npy_to_cann_desc(input_folder, output_folder, max_images=None):
    ...
    print(f"Tìm thấy {len(npy_files)} file .npy trong {input_folder}")
    ...
    for i, npy_file in enumerate(npy_files):
        if (i + 1) % 50 == 0:
            print(f"  Đang xử lý {i+1}/{len(npy_files)}: {npy_file.name}")
        ...
        try:
            ...
            print(f"  Lỗi xử lý {npy_file.name}: {e}")
    
    print(f"\n✓ Đã chuyển đổi {len(desc_files)} files sang .desc")
    ...
    print(f"✓ Đã tạo danh sách files: {list_file}")
...
def main():
    ...
    FEATURE_DIR = Path(BASE_DIR) / "data" / "feature" / "fire"
    
    datasets = [
        {
            'name': 'roxford5k',
            'input': FEATURE_DIR / 'roxford5k' / 'database',
            'output': Path(BASE_DIR) / 'cann_data' / 'roxford5k' / 'database'
        },
        {
            'name': 'roxford5k',
            'input': FEATURE_DIR / 'roxford5k' / 'query',
            'output': Path(BASE_DIR) / 'cann_data' / 'roxford5k' / 'query'
        },
        {
            'name': 'rparis6k',
            'input': FEATURE_DIR / 'rparis6k' / 'database',
            'output': Path(BASE_DIR) / 'cann_data' / 'rparis6k' / 'database'
        },
        {
            'name': 'rparis6k',
            'input': FEATURE_DIR / 'rparis6k' / 'query',
            'output': Path(BASE_DIR) / 'cann_data' / 'rparis6k' / 'query'
        }
    ]
    ...
    print("="*60)
    print("\nCác file .desc đã được tạo trong:")
